...
Use a DC-rated input fuse in an insulated assembly selected for the battery fault current, wiring and measured inrush; do not call the fuse servo-stall protection. Keep supply/return wires short and use heatshrink and strain relief. The extra external **220 µF bulk capacitor is omitted** from this POC. Retain the **100 nF ceramic bypass** at S2 VBUS/GND after `J_PWR`, and leave all capacitors already fitted to the buck in place. The small bypass does not replace bulk energy storage or establish that servo startup/reversal will keep the rail stable; validate the assembled supply under load. Use insulated soldered joints for power distribution. No perfboard, sensing transistors, sensing resistors, ADC filter, or GPIO3/GPIO7 wires are fitted.
...
1. **Inspect unpowered.** Confirm actual board labels, battery connector polarity, and the two converter input/output pairs. Check for supply-to-ground shorts. Leave wheels off and keep the XT30 disconnect accessible.
...
8. **Validate realistic combined load.** Exercise both servos and Wi-Fi together, measure current, observe supply dips and check the buck's temperature in its intended mounting. Test near 7 V input as well as at full input voltage. The 2.8 A budget and the decision to omit extra bulk capacitance remain unvalidated until these checks pass. If the rail dips or the S2 resets, stop the test and revisit the power design.
...
The former battery-sensing perfboard and all of its parts have been removed. The extra external 220 µF bulk capacitor and its printed holder are also removed. The retained 100 nF ceramic bypass connects directly across S2 VBUS and GND after `J_PWR`; keep the converter's onboard capacitors fitted. Neither the bypass nor the fuse provides charging or an automatic low-voltage power disconnect.
