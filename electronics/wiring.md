...
For routine receiver programming, use the [parked Wi-Fi update mode](../firmware/README.md#wireless-receiver-updates) after its initial USB installation. With **USB unplugged**, the battery, buck, `J_PWR`, and calibrated servo connections can stay in place; lift the wheels and verify behavior during reboot. The built-in BOOT/0 button enters maintenance after startup, so this adds no switch or wiring. The USB isolation instructions above and in the illustrated PDF remain necessary for initial setup and recovery. `J_PWR` is our name for the removable positive 5 V feed to VBUS, not a component built into the S2 Mini.
